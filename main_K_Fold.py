...
if args.fold:
    if args.model == 'cnn':
        train_func = getattr(train_K_Fold, 'train_CNN')
    elif args.model == 'cacnn1':
        train_func = getattr(train_K_Fold, 'train_CA_CNN_1')
    elif args.model == 'cacnn2':
        train_func = getattr(train_K_Fold, 'train_CA_CNN_2')
    elif args.model == 'resnet18':
        train_func = getattr(train_K_Fold, 'train_ResNet18')
    elif args.model == 'mobilenetv2':
        train_func = getattr(train_K_Fold, 'train_MobileNet_V2')
    elif args.model == 'mobilenetv3':
        train_func = getattr(train_K_Fold, 'train_MobileNet_V3')
    elif args.model == 'vit':
        train_func = getattr(train_K_Fold, 'train_ViT')
    elif args.model == 'deepvit':
        train_func = getattr(train_K_Fold, 'train_DeepViT')
    else:
        print("not found")

    # 调用相应的训练函数，并传入额外的参数
    train_func(args.epoch, args.bs, args.lr, args.data, args.fold)
